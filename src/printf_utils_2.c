...
/*   Updated: 2020/11/30 23:34:51 by bjacob           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_put_zero_and_space(long long i, int *tab, int len, int fd)
{
	...
	if (i < 0 && tab[2] > 0)
		diff_sign = 1;
	nb_space = ft_max(tab[4] - len, 0);
	if (tab[2] >= 0)
	{
		nb_zero = ft_max(tab[2] - (len - diff_sign), 0);
		if (len <= tab[2])
			nb_space = ft_max(ft_max(tab[0], tab[4]) - tab[2] - diff_sign, 0);
		else
			nb_space = ft_max(ft_max(tab[0], tab[4]) - len, 0);
	}
...
int	ft_put_zero_and_space_add(int *tab, int len, int fd)
{
	...
	nb_space = ft_max(ft_max(tab[0], tab[4]) - len - 2, 0);
	nb_zero = 0;
	if (tab[2] >= 0)
	{
